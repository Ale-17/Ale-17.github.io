...
let queued=false,refreshTimer=0,v63Ready=false,v64Ready=false;
function maybeLoadV66(){
  if(!v63Ready||!v64Ready||document.querySelector('script[data-fantasy-v66]'))return;
  const s=document.createElement('script');s.src='./fixes-v66.js?v=67';s.async=false;s.dataset.fantasyV66='1';document.body.appendChild(s);
}
function loadV64(){
  if(!document.querySelector('link[data-fantasy-v64]')){const l=document.createElement('link');l.rel='stylesheet';l.href='./fixes-v64.css?v=67';l.dataset.fantasyV64='1';document.head.appendChild(l)}
  const existing=document.querySelector('script[data-fantasy-v64]');
  if(existing){if(existing.dataset.loaded==='1'){v64Ready=true;maybeLoadV66()}else existing.addEventListener('load',()=>{existing.dataset.loaded='1';v64Ready=true;maybeLoadV66()},{once:true});return}
  const s=document.createElement('script');s.src='./fixes-v64.js?v=67';s.async=false;s.dataset.fantasyV64='1';s.addEventListener('load',()=>{s.dataset.loaded='1';v64Ready=true;maybeLoadV66()},{once:true});document.body.appendChild(s)
}
...
function loadV63(){
  if(!document.querySelector('link[data-fantasy-v63]')){const l=document.createElement('link');l.rel='stylesheet';l.href='./fixes-v63.css?v=63';l.dataset.fantasyV63='1';document.head.appendChild(l)}
  const existing=document.querySelector('script[data-fantasy-v63]');
  if(existing){if(existing.dataset.loaded==='1'){v63Ready=true;maybeLoadV66()}else existing.addEventListener('load',()=>{existing.dataset.loaded='1';v63Ready=true;maybeLoadV66()},{once:true});return}
  const s=document.createElement('script');s.src='./fixes-v63.js?v=63';s.async=false;s.dataset.fantasyV63='1';s.addEventListener('load',()=>{s.dataset.loaded='1';v63Ready=true;maybeLoadV66()},{once:true});document.body.appendChild(s)
}
